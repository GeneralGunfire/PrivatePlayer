...
export default function PlaylistPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handlePlay = () => {
    ...
    if (currentTrack?.id === first.id) {
      ...
      selectTrack(first, playlist.tracks);
      openPlayer();
    }
  };
    ...
    <div className="pb-32 pt-0 max-w-2xl mx-auto">
      ...
      <div className="relative overflow-hidden rounded-b-3xl mb-6">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-end gap-5 px-6 pt-16 pb-6">
          <div className="w-40 h-40 rounded-2xl overflow-hidden shadow-2xl shrink-0 glass">
      ...
      <div className="flex items-center gap-4 px-6 mb-6">
        <button
          ...
          className="w-14 h-14 rounded-full bg-white text-black flex items-center justify-center shadow-[0_0_40px_rgba(255,255,255,0.2)] hover:scale-105 active:scale-95 transition-all disabled:opacity-30"
        >
          {isPlaylistPlaying
        ...
        <button
          ...
          className={cn("transition-colors p-2", shuffle ? "text-white" : "text-white/30 hover:text-white")}
        >
          <Shuffle size={20} />
        </button>
      </div>
      ...
      <div className="px-4">
        {playlist.tracks.length === 0 ? (
          <div className="flex flex-col items-center py-16 text-center">
            <ListMusic className="w-12 h-12 text-white/15 mb-4" />
          ...
          <>
            ...
            <div
              ...
              style={{ gridTemplateColumns: "24px 48px 1fr 56px" }}
            >
              ...
              <Clock3 className="w-3 h-3 text-white/25 ml-auto" />
            </div>
            ...
            {playlist.tracks.map((track, i) => {
              ...
              return (
                <motion.div
                  key={track.id}
                  className={cn(
                    "grid items-center gap-4 px-3 py-2.5 rounded-xl cursor-pointer transition-colors",
                    hovering ? "bg-white/8" : isActive ? "bg-white/3" : ""
                  )}
                  style={{ gridTemplateColumns: "24px 48px 1fr 56px" }}
                ...
                >
                    ...
                    <AnimatePresence mode="wait">
                      {hovering
                        ? <motion.span key="play" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                            <Play className={cn("w-3.5 h-3.5 fill-current", isActive ? "text-white" : "text-white")} />
                          </motion.span>
                        : isRow
                          ? <motion.span key="bars" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-end gap-0.5 h-3.5 w-3.5">
                          ...
                          : <motion.span key="num" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                              className={cn("text-[11px] font-mono font-bold", isActive ? "text-white" : "text-white/30")}
                            >{i + 1}</motion.span>
                      }
                    </AnimatePresence>
                  </div>
                  ...
                  <div className="min-w-0">
                    <p className={cn("text-sm font-bold truncate uppercase tracking-tight", isActive ? "text-white" : "text-white")}>
                      {track.title}
                    </p>
                  ...
                  {/* Duration */}
                  <span className="text-[10px] font-mono text-white/30 text-right tracking-widest">{track.duration}</span>
                </motion.div>
              );
            })}
          </>
